...
/**
 * Custom hook to fetch uploaded educational documents status
 * @param {string} payrollId - The payroll ID (e.g., "RJY21")
 * @returns {object} React Query result with document paths
 */
export const useUploadedDocuments = (payrollId) => {
    return useQuery({
        queryKey: ["uploadedDocuments", payrollId],

        queryFn: async () => {
            const response = await axios.get(
                `http://localhost:8080/empDetails/HR/uploaded-educational-documents?payrollId=${payrollId}`
            );
            return response.data;
        },

        // Only fetch when payrollId is valid
        enabled: !!payrollId,
        retry: false,
    });
};
