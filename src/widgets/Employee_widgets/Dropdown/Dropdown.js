...
const Dropdown = ({
  ...
  // Normalize incoming list to { label, value } objects
  const normalized = useMemo(
    () => (Array.isArray(results) ? results : [])
      .filter((r) => r != null)
      .map((r) => {
        if (typeof r === 'object' && r !== null) {
          // Assuming object has label/value or name/id. 
          // Previous code uses mapToOptions in SkillTestForm which gives {label, value}
          const label = r.label !== undefined ? r.label : (r.name !== undefined ? r.name : String(r));
          const val = r.value !== undefined ? r.value : (r.id !== undefined ? r.id : r);
  ...
  const displayValue = selectedItem ? selectedItem.label : value; // Fallback to value if not found

  // Filter logic
  const filteredResults = useMemo(() => {
    // Show all options even if selected, or filter out selected? 
    // Original logic filtered out selected value. Let's keep that behavior.
    return normalized.filter((item) => item.value !== value);
  }, [normalized, value]);
  ...
  const listToShow = useMemo(() => {
    if (useFilter) {
      ...
      // Searching by label
      return filteredResults.filter((item) => item.label.toLowerCase().includes(lc));
    }
  ...
  const handleSelect = (option) => {
    // Pass just the value to onChange
    ...
    switch (e.key) {
      ...
      case "ArrowDown":
        ...
        setHighlightedIndex((prev) =>
          prev < listToShow.length - 1 ? prev + 1 : 0
        );
        break;
      case "ArrowUp":
        e.preventDefault();
        setHighlightedIndex((prev) =>
          prev > 0 ? prev - 1 : listToShow.length - 1
        );
        break;
      case "Enter":
        e.preventDefault();
  ...
  return (
    <div className={styles.dropdown_wrapper} ref={dropdownRef} onKeyDown={handleKeyDown}>
      ...
      >
        {displayValue || `Select ${dropdownname}`}
            ...
            {listToShow.length > 0 ? (
              listToShow.map((result, idx) => (
                ...
                >
                  <a
                    href="#"
                    className={`${styles.dropdown_option} ${idx === highlightedIndex ? styles.dropdown_option_highlighted : ""
                      }`}
